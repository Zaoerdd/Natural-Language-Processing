...
我基于 Shakespeare 语料实现了 `skip-gram + negative sampling` 的 Word2Vec 模型，并使用 `top-{teacher_topn}` 候选结果进行词类比评测。
...
从结果来看，**{best_run_name}** 的 analogy 表现优于另一组配置，因此我将它作为最终提交时重点展示的配置；另一组结果保留在报告中作为对照和补充说明。
...
def build_pdf(metrics):
    ...
    story = [
        ...
        Paragraph("1. 实验说明", heading_style),
        Paragraph(
            f"我基于 Shakespeare 语料完成了 skip-gram + negative sampling 的 Word2Vec 实现，并使用 top-{teacher_topn} 候选结果进行词类比评测。",
            body_style,
        ),
