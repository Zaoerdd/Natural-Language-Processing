...
我基于 Shakespeare 语料实现了 `skip-gram + negative sampling` 的 Word2Vec 模型，并使用 `top-5` 候选结果进行词类比评测。
...
从结果来看，**set1** 的 analogy 表现优于另一组配置，因此我将它作为最终提交时重点展示的配置；另一组结果保留在报告中作为对照和补充说明。
